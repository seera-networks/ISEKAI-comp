...
pub fn write_cert(
    ...
    let (cert_path, cert_str) = get_cert_path(path, cert_type, encoding, endorser);
...
pub fn get_vlek_cert_pem() -> Result<Vec<u8>> {
    ...
    if let Some(ref mut certificates) = certificates {
        for cert in certificates.iter() {
            if cert.cert_type == CertType::VLEK {
                let cert: Certificate = Certificate::from_bytes(&cert.data)?;
                return Ok(cert.to_pem()?);
            }
        }
    }

    return Err(anyhow::anyhow!("no VLEK certificate is available"));
}
