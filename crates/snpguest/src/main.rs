...
enum SnpGuestCmd {
    ...
    /// Verify2 command to fetch and verify attestation report.
    Verify2(verify2::Args),
...
async fn main() -> Result<()> {
    ...
    let status = match snpguest.cmd {
        ...
        SnpGuestCmd::Fetch(subcmd) => fetch::cmd(subcmd).await,
        SnpGuestCmd::Verify(subcmd) => verify::cmd(subcmd, snpguest.quiet),
        SnpGuestCmd::Verify2(args) => verify2::fetch_and_verify(args).await,
